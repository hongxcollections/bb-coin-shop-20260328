  ...
  return (
    <div className="rounded-lg border border-amber-100 bg-amber-50/50 overflow-hidden">
      ...
      <div className="px-3 pb-3 flex items-center gap-2 flex-wrap">
        ...
        {item.sellerWhatsapp && (
          <a
            href={toWhatsAppUrl(item.sellerWhatsapp, `您好，我在 hongxcollections 以 ${item.currency}$${Number(item.winningAmount).toLocaleString()} 得標「${item.title}」，想查詢付款及交收安排，謝謝！\n商品連結：https://share.hongxcollections.com/auctions/${item.id}`)}
            target="_blank"
            rel="noopener noreferrer"
        ...
        {item.sellerFacebook && (() => {
          ...
          const messengerLink = fbRaw.startsWith("http") ? fbRaw : `https://m.me/${fbRaw}`;
          const productUrl = `https://share.hongxcollections.com/auctions/${item.id}`;
          const msg = `您好，我在 hongxcollections 以 ${item.currency}$${Number(item.winningAmount).toLocaleString()} 得標「${item.title}」，想查詢付款及交收安排，謝謝！\n商品連結：${productUrl}`;
          const handleClick = async (e: React.MouseEvent) => {
            e.preventDefault();
            try {
              await navigator.clipboard.writeText(msg);
