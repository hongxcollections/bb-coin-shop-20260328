...
async function bootstrapMissingColumns() {
  ...
  if (!(await check('auctions', 'reservePrice'))) {
    await alter(`ALTER TABLE \`auctions\` ADD COLUMN \`reservePrice\` DECIMAL(10,2) NULL`,
  ...
  await alter(`CREATE TABLE IF NOT EXISTS \`pokeloverCards\` (
    \`id\` int NOT NULL AUTO_INCREMENT,
    \`userId\` int NOT NULL,
    \`cardName\` varchar(200) NULL,
    \`cardNameJa\` varchar(200) NULL,
    \`imageThumb\` mediumtext NULL,
    \`gradeEstimate\` int NULL,
    \`bgsEstimate\` float NULL,
    \`cgcEstimate\` float NULL,
    \`tagEstimate\` int NULL,
    \`condition\` varchar(50) NULL,
    \`marketPriceHKD\` int NULL,
    \`psa9HKD\` int NULL,
    \`psa10HKD\` int NULL,
    \`cardSet\` varchar(200) NULL,
...
async function startServer() {
  ...
  app.use(
    helmet({
      // CSP 暫時停用：code splitting 動態 chunk 需要更仔細的 nonce/hash 設定
      // 保留其他安全標頭（HSTS, X-Frame-Options, X-Content-Type-Options 等）
...
Output ONLY the JSON, nothing else.`;
  ...
  app.get('/api/og-image-card-wtb/:wtbId', async (req, res) => {
    try {
      const wtbId = parseInt(req.params.wtbId, 10);
      if (isNaN(wtbId) || wtbId <= 0) { res.status(400).send('Invalid WTB ID'); return; }
      const { getRawPool } = await import('../db') as any;
      const pool = await getRawPool();
      const [rows]: any = await pool.execute(
        'SELECT photoUrlsJson, officialImageUrl FROM cardWantToBuy WHERE id = ? LIMIT 1',
        [wtbId]
      );
      const row = (rows as any[])[0];
      if (!row) { res.status(404).send('WTB not found'); return; }
      ...
      console.error('[OG Image Card WTB Proxy] Error:', err);
      res.status(500).send('Error');
    }
  });
