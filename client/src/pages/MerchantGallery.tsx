...
export default function MerchantGallery() {
  ...
  // 'list' = return to gallery list, 'gallery' = return to public gallery page, null = stay in edit view
  const [posterCancelNav, setPosterCancelNav] = useState<'list' | 'gallery' | null>(null);
  ...
  const [poolItemEditName, setPoolItemEditName] = useState('');
  const [poolItemEditNumber, setPoolItemEditNumber] = useState('');
  const [poolItemEditPrice, setPoolItemEditPrice] = useState('');
  ...
  const distributeImagesToItemsM = trpc.productGalleries.distributeImagesToItems.useMutation({
    onSuccess: (data) => {
      const msg = `已完成，共 ${data.assigned} 次分配`;
      toast.success(msg);
  ...
  const updateItemM = trpc.productGalleries.updateItem.useMutation({
    onSuccess: () => { getForEditQ.refetch(); },
    onError: (e) => toast.error(e.message),
  });
                ...
                <div>
                  ...
                  {assignPickerImageId !== null && (
                    ...
                    >
                      <div
                        className="bg-white w-full rounded-t-2xl px-4 pt-4"
                        style={{ maxHeight: '65vh', overflowY: 'auto', paddingBottom: 'calc(100px + env(safe-area-inset-bottom, 0px))' }}
                        onClick={e => e.stopPropagation()}
                      >
                  ...
                  {distributeItemPickerOpen && (() => {
                    ...
                    return (
                      ...
                      >
                        <div
                          className="bg-white w-full rounded-t-2xl px-4 pt-4"
                          style={{ maxHeight: '75vh', display: 'flex', flexDirection: 'column', paddingBottom: 'calc(100px + env(safe-area-inset-bottom, 0px))' }}
                          onClick={e => e.stopPropagation()}
                        >
                          ...
                          <p className="text-xs text-gray-400 mb-3 flex-shrink-0">
                            已選 {poolImgIds.length} 張圖片，全部複製到每件選定商品（每件商品都有齊 {poolImgIds.length} 張）
                          </p>
                  ...
                  {poolItemEditOpen && poolItemEditItem && (() => {
                    ...
                    const pendingImgs = (galleryImagesQ.data ?? []).filter(
                    ...
                    return (
                      <div key={order.id} className="bg-white rounded-2xl p-4 space-y-2 shadow-sm">
                        ...
                        <div className="pt-1 border-t border-gray-50 space-y-2">
                          ...
                          {/* Contact buyer + order message copy */}
                          {(() => {
                            const orderDate = order.createdAt ? new Date(order.createdAt) : new Date();
                            const ymd = orderDate.toLocaleDateString('sv-SE', { timeZone: 'Asia/Hong_Kong' });
                            const hm = orderDate.toLocaleTimeString('en-GB', { timeZone: 'Asia/Hong_Kong', hour: '2-digit', minute: '2-digit' });
                            const itemUrl = `https://hongxcollections.com/gallery/${order.galleryId}?item=${order.galleryItemId}`;
                            const priceStr = `${order.currency} $${(parseFloat(order.price) || 0).toLocaleString('en-HK', { minimumFractionDigits: 0 })}`;
                            const merchantNameStr = (currentGallery as any)?.merchantName ?? '';
                            const orderMsg = [
                              order.title || `訂單 #${order.id}`,
                              `價錢 ${priceStr}`,
                              itemUrl,
                              '多謝購買 請聯繫及安排交收事宜🤝',
                            ...
                            return (
                              <div className="flex gap-2 flex-wrap">
                                {waLink && (
                                  <a
                                    href={`${waLink}?text=${encodeURIComponent(orderMsg)}`}
                                    target="_blank"
              ...
              >
                <button
                  onClick={() => {
                    ...
                    if (posterCancelNav === 'list') goList();
                    else if (posterCancelNav === 'gallery') navigate(`/gallery/${editGalleryId}`);
                  }}
