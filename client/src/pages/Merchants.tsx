...
import { Store, Package, X, ChevronLeft, ChevronRight, ShoppingCart, Loader2, CheckCircle2 } from "lucide-react";
      ...
      <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/50 p-4 pb-20" onClick={onClose}>
        <div className="bg-white rounded-2xl p-6 w-full max-w-sm text-center" onClick={e => e.stopPropagation()}>
          ...
          <div className="bg-amber-50 rounded-xl px-4 py-2.5 mb-4 text-left">
            ...
            <p className="text-amber-600 font-bold text-sm mt-0.5">{product.currency ?? "HKD"} ${(price * orderedQty).toLocaleString()}</p>
          </div>
...
function ProductCard({ p, layout, whatsapp, messengerLink, onBuy, currentUserId }: { p: any; layout: LayoutMode; whatsapp: string; messengerLink: string; onBuy: (p: any) => void; currentUserId?: number | null }) {
  ...
  const buyBtn = isSold ? (
    <span className="flex items-center gap-1 bg-gray-100 text-gray-500 text-xs font-semibold px-2.5 py-1.5 rounded-full shrink-0">已售出</span>
  ) : isOwner ? (
    ...
    <Link href={href}>
      <div className={`bg-white rounded-xl border shadow-sm p-3 flex gap-3 items-start cursor-pointer transition-colors ${isSold ? "border-gray-100 opacity-75 hover:border-gray-200" : "border-amber-100 hover:border-amber-300"}`}>
        <div className="relative shrink-0">
          {imgs[0] ? <img src={imgs[0]} alt={p.title} className="w-16 h-16 rounded-lg object-cover" />
...
function MerchantSection({ merchant, selectedCategory, onBuy }: { merchant: any; selectedCategory: string; onBuy: (p: any) => void }) {
  ...
  if (!isLoading && visible.length === 0 && soldProducts.length === 0) return null;
        ...
        <>
          ...
          {soldProducts.length > 0 && (
            <div className="mt-4">
              ...
              <ProductsGrid products={soldProducts} layout={layout} whatsapp={merchant.whatsapp ?? ""} messengerLink={messengerLink} onBuy={onBuy} currentUserId={user?.id} />
            </div>
          )}
        </>
      )}
